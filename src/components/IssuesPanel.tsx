...
export function IssuesPanel({ issues }: IssuesPanelProps) {
  return (
    <aside className="theme-surface grid gap-3 rounded-xl border !border-[var(--color-border)] p-4" aria-label="issues-panel">
      ...
      {issues.length === 0 ? (
        ...
        <ul className="m-0 grid list-none gap-2 p-0">
          {issues.map((issue) => (
            <li key={issue.id} className="grid gap-1 rounded-md border !border-[var(--color-border)] bg-amber-50 p-3 dark:bg-amber-950">
